import { CUSTOM_ELEMENTS_SCHEMA } from '@angular/core';
import { async, ComponentFixture, TestBed } from '@angular/core/testing';

import { UpdateserviceprovidercontactinfoPage } from './updateserviceprovidercontactinfo.page';

describe('UpdateserviceprovidercontactinfoPage', () => {
  let component: UpdateserviceprovidercontactinfoPage;
  let fixture: ComponentFixture<UpdateserviceprovidercontactinfoPage>;

  beforeEach(async(() => {
    TestBed.configureTestingModule({
      declarations: [ UpdateserviceprovidercontactinfoPage ],
      schemas: [CUSTOM_ELEMENTS_SCHEMA],
    })
    .compileComponents();
  }));

  beforeEach(() => {
    fixture = TestBed.createComponent(UpdateserviceprovidercontactinfoPage);
    component = fixture.componentInstance;
    fixture.detectChanges();
  });

  it('should create', () => {
    expect(component).toBeTruthy();
  });
});
